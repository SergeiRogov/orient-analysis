...
import { OrientCourse, Runner, SplitContext } from "./../App";
...
export const Graphs = ({ orientCourse }: Props) => {
  ...
  const bestCumulSelected = splitData.controls[orientCourse.key].map(
    (_, index) => {
      return runnersToDisplay.reduce((minValue, runner) => {
        const cumulTime: number = runner?.splits?.[index]?.[6];
        return cumulTime !== undefined
          ? Math.min(minValue, cumulTime)
          : minValue;
      }, Infinity);
    }
  );

  const worstCumulSelected = splitData.controls[orientCourse.key].map(
    (_, index) => {
      return runnersToDisplay.reduce((maxValue, runner) => {
        const cumulTime: number = runner?.splits?.[index]?.[6];
        return cumulTime !== undefined
          ? Math.max(maxValue, cumulTime)
          : maxValue;
  ...
  const drawGraph = (ctx: CanvasRenderingContext2D) => {
    ...
    verticalLinesCoordinates.slice(1).forEach((x: number, index: number) => {
      ctx.fillText(
        index === verticalLinesCoordinates.length - 2
          ? "F"
          : (index + 1).toString(),
        index === verticalLinesCoordinates.length - 2
          ? MARGIN_LEFT + graphWidth + 2
          : x + MARGIN_LEFT - 3,
    ...
    for (let i = 0; i <= BEHIND_LINES_COUNT; i++) {
      ...
      if (selectedRunners[orientCourse.key].length > 1) {
        ...
        ctx.font = "13px Open Sans";
        ctx.fillText(
          "+" + secondsToTime(Math.floor(i * timeInterval)).toString(),
          MARGIN_LEFT / 5,
          i * interval + MARGIN_TOP + 5
        );
      }
    }
    let Ycoordinate = 0;
    ...
    runnersToDisplay.forEach((runner: Runner, runnerIndex) => {
      ...
      // text with names
      if (
        selectedRunners[orientCourse.key].length > 1 &&
        ...
        if (
          (runner.id > firstDNFid &&
            /^\d{1,3}:\d{2}$/.test(runner.overall_time)) ||
          (runnerIndex > 0 &&
            timeToSeconds(runnersToDisplay[runnerIndex - 1]?.overall_time) >
              timeToSeconds(runner.overall_time))
        ) {
          ...
          runnerIndex > 0 &&
          prevYcoordinate + 13 > curentYcoordinate
        ) {
