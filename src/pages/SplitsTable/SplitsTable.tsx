...
function calculateTimeDifference(
  ...
  const formattedDifference = `${sign}${String(differenceMinutes).padStart(
    2,
    "0"
...
export const SplitsTable = ({ orientCourse, isLoading }: Props) => {
  ...
  const splitColumns: Column<Runner>[] = splitData.controls[
    orientCourse.key
  ].map((control: string, index: number) => ({
    Header: control,
    accessor: (row) => {
    ...
    Cell: ({ cell: { value }, row }: any) => {
      if (typeof value === "string") {
        ...
        let linesHTML = lines.map((line: string, index: number) => {
          if (tuple) {
            const isBoldLine1 =
              index === 0 && typeof tuple[1] === "number" && tuple[1] === 1;
            const isBoldLine2 =
              index === 1 && typeof tuple[2] === "number" && tuple[2] === 1;
            const isBlue1 =
              index === 0 &&
            ...
            return (
              <div
                ...
                style={{
                  ...
                  textDecoration:
                    isBoldLine1 || isBoldLine2 ? "underline" : "none",
                  color: isBlue1 || isBlue2 ? "blue" : "black",
                  backgroundColor:
                    index === 0 ? getColor(tuple[3]) : getColor(tuple[4]),
    ...
    () => [
      {
        ...
        columns: [
          ...
          { Header: "Result", accessor: "overall_time" },
          {
            Header: "Behind",
            accessor: (row) => {
              const firstRunnerResult =
                splitData.runners[orientCourse.key][0].overall_time;
  ...
  const data = useMemo(
    () => splitData.runners[orientCourse.key],
    [orientCourse, splitData]
  );
  ...
  return (
    <div
      ...
      style={{
        maxHeight: maxHeight ? `${maxHeight}px` : "1700px",
        overflowY: "auto",
      }}
    >
      <table
