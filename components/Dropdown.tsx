...
import { Option, OrientCourse, SplitContext } from "../App";
...
export const Dropdown = ({
  ...
  useEffect(() => {
    if (splitData) {
      const defaultOrientCourse =
        splitData.courses.length > 0
          ? { name: splitData.courses[0], key: 0 }
          : { name: "Blue", key: 0 };
    ...
    <div className="Event-Course-Menu">
      ...
      >
        {options.map((opt: Option) => (
          <option value={opt.value} style={{ textAlign: "center" }}>
            {opt.label}
          </option>
        ))}
      </select>
      ...
      <div className="label">
        <h4 className="label course" style={{ marginRight: "10px" }}>
          Course:
        </h4>
      </div>
      ...
      >
        {splitData.courses.map((course: string) => (
          <option key={course} style={{ textAlign: "center" }}>
            {course}
          </option>
        ))}
      </select>
